...
start_date_epoch=$(date -j -f "%Y-%m-%d %H:%M:%S" "$start_date" +"%s")
end_date_epoch=$(date -j -f "%Y-%m-%d %H:%M:%S" "$end_date" +"%s")
...
for i in $(seq 1 $total_commits); do
    ...
    random_date=$(date -j -f "%s" "$random_date_epoch" +"%Y-%m-%d %H:%M:%S")
